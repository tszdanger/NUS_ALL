...
def yuyin():
    ...
    wf.close()
    model = load_model('asr_model_2_happy.h5')  # 加载训练模型
    wavs = []
...
b = tk.Button(window,text = '语音识别',font = ('Arial',12),width=10,height = 1,command = yuyin)
b.pack()
var3 = tk.StringVar()
var4 = tk.StringVar()
...
c = tk.Button(window,text = '图片处理',font = ('Arial',12),width=10,height = 1,command = tupian)
c.pack()
...
# # 说明图片位置，并导入图片到画布上
# image_file = tk.PhotoImage(file='3_new.png')  # 图片位置（相对路径，与.py文件同一文件夹下，也可以用绝对路径，需要给定图片具体绝对路径）
